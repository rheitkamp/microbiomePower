...
subjects=c(10,25,35,55) #number of subjects per group
...
power=apply(range,1,function(x){Nrsg = list();
					  Nrsg[[1]]=rep(x[2], x[1]); 
					  Nrsg[[2]]=rep(x[2],balance*x[1]);
					  MC.Xmcupo.statistics(Nrs=Nrsg, MC, pi0=fit2$pi, group.pi=pi_2gr, group.theta=group.theta, type = "ha", siglev = 0.05)})
...
rownames(power.sample.size.table)<-paste(as.numeric(rownames(power.sample.size.table)),as.numeric(rownames(power.sample.size.table))*balance,sep=",")
power.sample.size.table
